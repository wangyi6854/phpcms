...
class GroupList extends App
{
	...
	public function getListByStartId( $parts )
	{
		...
		$query = "select * from group $where order by $this->order limit $size";
	...
	public function getListByPage( $parts )
	{
		...
		$query = "select sql_calc_found_rows * from group $where order by $this->order";
	...
	private function initCondition( $condition )
	{
		...
		$this->cacheKey = "grouplist_{$this->site}_{$this->page}_{$this->startId}_{$this->pageSize}" . md5( $this->order );
	}
