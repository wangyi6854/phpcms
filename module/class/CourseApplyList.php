...
class CourseApplyList extends App
{
	public $page			= 0;
	public $startId			= 0;
	public $pageSize		= 20;
	...
	public function getList()
	{
		...
		else
		{
			...
			$this->site ? $parts[] = "site = " . $this->db->quote( "$this->site" ) : null;
	...
	public function getListByStartId( $parts )
	{
        ...
        $parts[] = "a.courseId = c.id and a.userId = u.id";

		$where = $this->buildWhere( $parts );

		$size = $this->pageSize + 1;

		$query = "select a.id, a.ts, c.*, u.*  from course_apply a, course c, user u $where order by $this->order limit $size";
	...
	public function getListByPage( $parts )
	{
		...
		$query = "select sql_calc_found_rows a.id, a.ts, c.*, u.*  from course_apply a, course c, user u  $where order by $this->order";
	...
	private function initCondition( $condition )
	{
		...
		$this->cacheKey = "newslist_{$this->site}_{$this->page}_{$this->startId}_{$this->pageSize}" . md5( $this->order );
	}
