...
class SiteList extends App
{
	...
	public function getListByStartId( $parts )
	{
		...
		$query = "select * from site $where order by $this->order limit $size";
	...
	public function getListByPage( $parts )
	{
		...
		$query = "select sql_calc_found_rows * from site $where order by $this->order";
	...
	private function initCondition( $condition )
	{
		...
		$this->cacheKey = "sitelist_{$this->page}_{$this->startId}_{$this->pageSize}" . md5( $this->order );
	}
